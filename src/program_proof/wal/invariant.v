...
Theorem memLog_linv_implies_is_mem_memLog γ (σ: locked_state) :
  memLog_linv γ σ.(memLog) σ.(diskEnd) σ.(locked_diskEnd_txn_id) -∗
  memLog_linv γ σ.(memLog) σ.(diskEnd) σ.(locked_diskEnd_txn_id) ∗
...
Proof.
  ...
  iNamed "HmemLog".
  iNamed "Htxns".
  epose proof (has_updates_app _ _ _ _ His_memStart His_nextDiskEnd) as Hhas_updates_mid.
  ...
  ∃ (new_installed_txn_id: nat) (being_installed: gset Z),
    ...
    "Howninstalled" ∷ (
      ...
      "Hbeing_installed_txns" ∷ ghost_var γ.(being_installed_txns_name) (1/2)
        (subslice installed_txn_id new_installed_txn_id txns)) ∗
